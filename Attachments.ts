import { Color, ComputedAttachment, Entity, EntityManager, Input, RendererSDK, Vector2 } from "github.com/octarine-public/wrapper/index"
...
function RenderAttachment(ent: Entity, attach: ComputedAttachment, color: Color, name: string): void {
	const time = !GameTimeState.value
		? (attach.FrameCount / attach.FPS) / 2
		: ent.AnimationTime
	...
	if (screen_pos === undefined)
		return
	if (Input.CursorOnScreen.Distance(screen_pos) < 16)
		RendererSDK.Text(
			name,
			screen_pos,
			color,
