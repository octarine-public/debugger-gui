...
function UnitPath(name: string): string {
	if (name.includes("npc_dota_hero_")) return `panorama/images/heroes/${name}_png.vtex_c`
	...
	if (name.includes("eidolon")) return "panorama/images/heroes/npc_dota_eidolon_png.vtex_c"
	...
	if (name.includes("golem")) return "panorama/images/heroes/npc_dota_warlock_golem_png.vtex_c"
	...
	if (name.includes("lycan_wolf")) return "panorama/images/heroes/npc_dota_lycan_wolf_png.vtex_c"
...
function GetEntityTeamColor(ent: Entity): Color {
	if (ent.IsNeutral) return Color.White
	if (ent.IsEnemy()) return Color.Red
	return Color.Green
}
...
function DrawTrees(size: Vector2): void {
	...
	for (const tree of Trees) {
		...
		if (screen_pos === undefined) continue
		screen_pos.SubtractForThis(tree_size.DivideScalar(2))
		RendererSDK.FilledCircle(screen_pos, tree_size, Color.Yellow)
	}
}
...
function DrawCreeps(size: Vector2): void {
	for (const creep of Creeps) {
		if (/*!creep.IsVisible || */ !creep.IsAlive /* || !creep.IsSpawned*/) continue
...
function DrawHeroes(size: Vector2): void {
	for (const hero of Heroes)
		if (hero.IsVisible && hero.IsAlive) RenderEntity(hero, size, UnitPath(hero.Name))
}

const Runes = EntityManager.GetEntitiesByClass(Rune)
function DrawRunes(size: Vector2): void {
	for (const rune of Runes) {
		if (!rune.IsVisible) continue
		...
		RenderEntity(
			rune,
			size,
			`github.com/octarine-private/immortal-core/scripts_files/images/runes/${rune_name}.png`
		)
	}
}
...
function DrawOther(): void {
	for (const ent of otherEnts) {
		if (!ent.IsVisible || (ent instanceof Unit && !ent.IsSpawned)) continue
		const screen_pos = RendererSDK.WorldToScreen(ent.Position)
		if (screen_pos === undefined) continue
		let text = ent.Name
		if (text.length === 0) text = ent.ClassName
		RendererSDK.Text(text, screen_pos, GetEntityTeamColor(ent))
	}
}
...
function DrawHitboxes(): void {
	EntityManager.AllEntities.forEach(ent => {
		...
		if (ent instanceof Tree && (!TreesState.value || !ent.IsAlive)) return
		else if (ent instanceof Creep && (!CreepsState.value || !ent.IsAlive || !ent.IsSpawned))
			return
		else if (ent instanceof Rune && !RunesState.value) return
		else if (ent instanceof Hero && (!HeroesState.value || !ent.IsAlive)) return
...
export function DrawEntities(): void {
	...
	if (Hitboxes.value) DrawHitboxes()
	if (!Icons.value) return
	if (TreesState.value) DrawTrees(size)
	if (CreepsState.value) DrawCreeps(size)
	if (RunesState.value) DrawRunes(size)
	if (HeroesState.value) DrawHeroes(size)
	if (Other.value) DrawOther()
}
