...
export function DrawFPS(): void {
	...
	if (last_frames_calls.length === stored_frame_count) {
		for (let i = 1; i < stored_frame_count; i++) last_frames_calls[i - 1] = last_frames_calls[i]
	} else last_frames_calls.push(0)
	...
	RendererSDK.Text(
		text,
		new Vector2(screen_size.x - text_size.x - x_offset, y_offset),
		Color.Yellow,
